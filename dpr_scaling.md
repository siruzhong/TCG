# DPR Wide Scaling Study

Generated from `checkpoints/test_scaling/` aggregation. Base results from `dpr_result.md`.
...
- **param_match**: width auto-tuned so base (no DPR) params ≈ base_dpr params; skipped if deviation >50%

## PatchTST

| dataset      | horizon | base_raw | base_dpr | w2_d1     | w1_d2     | w2_d2     | param_match |
| ------------ | ------- | -------- | -------- | --------- | --------- | --------- | ----------- |
...
| dataset      | horizon | base_raw | base_dpr | w2_d1     | w1_d2     | w2_d2     | param_match |
| ------------ | ------- | -------- | -------- | --------- | --------- | --------- | ----------- |
| ETTh1        | 96      | 0.488 / 0.475 | 0.475 / 0.466 | 0.561 / 0.509 | 0.503 / 0.481 | 0.598 / 0.518 | 0.488 / 0.475 |
...
1. **base_raw** = model without DPR, base config (from `dpr_result.md`)
2. **base_dpr** = model with DPR, base config (from `dpr_result.md`)
3. **param_match** = base model sans DPR, width auto-tuned to equal base_dpr's parameter count (fair budget comparison). For models where DPR adds negligible params (e.g. PatchTST: 18.5M → 18.5M), param_match ≈ base_raw by design.
4. **w2_d1** (2x width): Performance is dataset/model dependent; gains are not consistent.
5. **w1_d2** (2x depth): Usually limited benefit, and can be worse than base_dpr.
...
8. **param_match vs base_dpr (fair comparison)**:
   - PatchTST: param_match ≈ base_raw ≈ base_dpr on all datasets; DPR doesn't help but also doesn't hurt.
   - TimesNet: On ExchangeRate, param_match (0.126) ≈ base_dpr (0.128) — at equal params, DPR roughly ties with scaling. On Illness, base_dpr (6.77) ≫ param_match (9.35) — DPR substantially outperforms pure width scaling.
   - TimeFilter: On ETTh1 and ExchangeRate, param_match ≈ base_dpr ≈ base_raw. On Illness, base_dpr (1.82) beats param_match (2.81).
