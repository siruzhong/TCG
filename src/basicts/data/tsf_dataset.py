...
class BasicTSForecastingDataset(BasicTSDataset):
    ...
    def __init__(
            ...
            memmap: bool = False,
            use_clean_targets: bool = False) -> None:
        """
        ...
        Args:
            ...
            memmap (bool): Flag to determine if the dataset should be loaded using memory mapping.
            use_clean_targets (bool): If True, use clean data (from {mode}_data_clean.npy) as targets while inputs remain noisy.
                                     This is useful for denoising tasks where you want to learn to predict clean signals from noisy inputs.
        """
        ...
        try:
            ...
            # Load clean targets if requested
            if use_clean_targets:
                clean_data_path = os.path.join(data_file_path, f"{mode}_data_clean.npy")
                if os.path.exists(clean_data_path):
                    self._clean_data = np.load(clean_data_path, mmap_mode="r" if memmap else None)
                    print(f"[{mode.upper()}] Successfully loaded CLEAN targets from {clean_data_path}")
                else:
                    # Fallback to noisy data if clean data not found
                    self._clean_data = None
                    print(f"[{mode.upper()}] WARNING: Clean targets requested but {clean_data_path} not found. Fallback to NOISY targets.")
            else:
                self._clean_data = None
            if use_timestamps:
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        # Use clean data for targets if available, otherwise use noisy data
        target_data = self._clean_data if self.use_clean_targets and self._clean_data is not None else self._data
        future_data = target_data[index + self.input_len: index + self.input_len + self.output_len]
